<template>
  <div class="voting">
    <div class="container text-left">
      <img src="" alt="" />
      <h1 class="text-white tittle">PEMIRA FP UNS 2020</h1>
      <h4 class="text-white mt-1 mb-5">
        Halo {{ participant.name }}, Silakan Ketuk Pilih untuk memilih daftar
        Calon Legislatif Program Studi dibawah ini
      </h4>
      <b-row>
        <b-col
          lg="4"
          class="mb-5 text-center"
          v-for="candidate in Legislatif2Candidates"
          :key="candidate._id"
        >
...
export default {
  ...
  methods: {
    vote(id_candidate_legislatif2, name_candidate) {
      new Swal({
        ...
        if (result.isConfirmed) {
          let data = {
            ...
            id_candidate_legislatif: this.id_candidate_legislatif,
  ...
  created() {
    if (this.$route.query.id_candidate_bem) {
      ...
      this.id_candidate_legislatif = this.$route.query.id_candidate_legislatif;
    }
  },
  computed: {
